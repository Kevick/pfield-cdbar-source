...
bool argumentsHandler(StringVec args)
{
	...
	for(StringVec::iterator it = args.begin(); it != args.end(); ++it)
	{
		...
		if((*it) == "--version")
		{
			...
			"A server developed by Elf, slawkens, Talaturen, Lithium, KaczooH, Kiper, Kornholijo, Max William, Valakinhas.\n"
...
ServiceManager* services)
{
	...
	std::cout << "A server developed by YuriAntunes." << std::endl;
	std::cout << "Visite o forum para updates e resource: http://otland.net." << std::endl << std::endl;
	...
	if(GetLastError() == ERROR_ALREADY_EXISTS)
		startupErrorMessage("Outro Executavel ja esta em uso.");
			...
			switch(LOWORD(wParam))
			{
				...
				case ID_MENU_MAIN_CLEARLOG:
				{
					if(g_game.getGameState() != GAME_STATE_STARTUP)
					{
						...
						std::cout << STATUS_SERVER_NAME << ", version " << STATUS_SERVER_VERSION << " (" << STATUS_SERVER_CODENAME << ")" << std::endl;
						std::cout << "Compiled with " << BOOST_COMPILER << " at " << __DATE__ << ", " << __TIME__ << "." << std::endl;
						std::cout << "A server developed by Elf, slawkens, Talaturen, Lithium, KaczooH, Kiper, Kornholijo,Max William ,Valakinhas." << std::endl;
						std::cout << "Visite o forum para updates ou resource: http://otland.net." << std::endl;
						std::cout << std::endl;
					}
				...
				{
					if(MessageBox(hwnd, "Voce tem certesa que quer desligar o servidor ?", "Shutdown", MB_YESNO) == IDYES)
					{
